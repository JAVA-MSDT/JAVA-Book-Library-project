...
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/css/formStyle.css">
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/css/accountBodyStyle.css">
<script rel="script"
	src="${pageContext.request.contextPath}/js/orderValidator.js"></script>
</head>
...
<body>
	...
	<div class="profileContainer">
		<div class="basicInfo">
			...
			<h1>
				<fmt:message key="label.add.edit.order" />
			</h1>
			<hr>
			...
			<c:choose>
				...
				<c:when test="${ param.operationStatus eq 'updated'}">
					<h2 class="permission" style="color: green; margin: 20px auto">
						<fmt:message key="message.update.done" />
					</h2>
					<br>
				</c:when>
				<c:when test="${ param.operationStatus eq 'updateFail'}">
					<h2 class="permission" style="color: green; margin: 20px auto">
						<fmt:message key="message.update.not.done" />
					</h2>
					<br>
				</c:when>
				...
				<c:when test="${param.operationStatus eq 'inserted'}">
					<h2 class="permission" style="color: green; margin: 20px auto">
						<fmt:message key="message.insert.done" />
					</h2>
					<br>
				</c:when>
				<c:when test="${param.operationStatus eq 'insertFail'}">
					<h2 class="permission" style="color: green; margin: 20px auto">
						<fmt:message key="message.insert.not.done" />
					</h2>
					<br>
				</c:when>
				<%-- Removing info--%>
					...
					<c:choose>
						<c:when test="${validation eq 'returningDateOlder'}">
							<h3 style="color: firebrick">
								<fmt:message key="validation.order.returning.date" />
							</h3>
						</c:when>
					</c:choose>
				</c:forEach>
			...
			<div class="container">
				<div class="editContainerForm">
					<form id="librarianOrderForm" name="administration-update-order"
						action="controller" method="post">
						<input type="hidden" name="command"
							value="administration-update-order"> <input type="hidden"
							name="insert" value="${sessionScope.inserted}"> <input
							type="hidden" name="id" value="${requestScope.editOrder.orderId}">
						<input type="hidden" id="returned-message"
							value="<fmt:message key="message.remove.order"/>">
						...
						<div class="row">
							<div class="labelCol">
								...
								<p id="returning-date-message" style="display: none">
									<fmt:message key="validation.order.returning.date" />
								</p>
							</div>
							<div class="inputCol">
								<input type="date" name="returning_date" id="returning-date"
									...
									placeholder="<fmt:message key="label.order.return.date"/>"
									required>
						...
						<div class="row">
							...
							<div class="inputCol">
								<select id="book-returned" name="book_returned">
									<option
										value=" ${requestScope.editOrder.bookReturned eq  'false' ? 'false' : 'false'}"
										<c:if test="${requestScope.editOrder.bookReturned eq  'false' }"> selected = "selected" </c:if>>
										<fmt:message key="label.false" />
									</option>
									<option
										value="${requestScope.editOrder.bookReturned eq 'true' ? 'true' : 'true'}"
