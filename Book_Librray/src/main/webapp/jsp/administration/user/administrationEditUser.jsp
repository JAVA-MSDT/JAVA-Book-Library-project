...
<script rel="script"
	src="${pageContext.request.contextPath}/js/userValidator.js"></script>
</head>
...
<body>
	...
	<div class="profileContainer">
		<div class="basicInfo">
			...
			<h1>
				<fmt:message key="label.add.edit.user" />
			</h1>
			...
			<c:choose>
				...
				<c:when test="${param.operationStatus eq 'insertFail'}">
					<h2 class="permission" style="color: green; margin: 20px auto">
						<fmt:message key="message.insert.user.not.done" />
					</h2>
					<br>
				</c:when>
			</c:choose>
					...
					<c:choose>
						<c:when test="${validation eq 'nameError'}">
							<h3 style="color: firebrick">
								<fmt:message key="validation.user.name" />
							</h3>
						</c:when>
						...
						<c:when test="${validation eq 'loginError'}">
							<h3 style="color: firebrick">
								<fmt:message key="validation.user.login" />
							</h3>
						</c:when>
			...
			<div class="container">
				<div class="editContainerForm">
					<form id="librarianReaderForm" name="administration-update-user"
						action="controller" method="post">
						<input type="hidden" name="command"
							value="administration-update-user"> <input type="hidden"
							name="id" value="${requestScope.editUser.id}">
						<div class="row">
							<div class="labelCol">
								<h3 class="label">
							...
							<div class="inputCol">
								<input type="text" name="name" id="user-name"
									...
									pattern="([\w\s]{0,30})$"
									placeholder="<fmt:message key="validation.user.name"/>"
									required>
							</div>
						</div>
						<div class="row">
						...
						<div class="row">
							...
							<div class="inputCol">
								<input type="email" name="email" id="email"
									...
									placeholder="<fmt:message key="validation.user.email"/>"
									required>
							</div>
						</div>
						<div class="row">
							...
							<div class="inputCol">
								<input type="text" name="login" id="user-login"
									...
									pattern="^([a-zA-Z]{2,10})$"
									placeholder="<fmt:message key="validation.user.login"/>"
									required>
							</div>
						</div>
							...
							<div class="row">
								...
								<div class="inputCol">
									<input type="text" name="password" id="user-password"
										...
										placeholder="<fmt:message key="validation.user.password"/>"
										required>
								</div>
							</div>
						</c:if>
						...
						<div class="row">
							<input type="submit" onclick="return userFormValidation()"
