	...
	<div class="main-container">
		...
		<c:choose>
			<c:when test="${not empty requestScope.invalidLogin}">
				<h2 class="permission">
					<fmt:message key="message.login.register" />
				</h2>
				<br>
			</c:when>
			<c:when test="${not empty requestScope.operationStatus}">
				<h2 class="permission" style="color: darkred">
					<fmt:message key="message.book.order.not.done" />
				</h2>
				<br>
			</c:when>
		</c:choose>
			...
			<div class="book-description">
				...
				<div class="order-button-container" style="margin-top: 25px">
					<form name="order-book" action="controller" method="post">
						<input type="hidden" name="command" value="order-book"> <input
							style="width: 200px" class="edit" type="submit" name="order"
							value="<fmt:message key="button.order"/> " /> <input
							type="hidden" name="id"
		...
		<c:if test="${not empty requestScope.display}">
			<div class="container">
				...
				<div class="editContainerForm">
					<form id="librarianOrderForm" name="confirm-order"
						...
						<div class="row">
							...
							<div class="inputCol">
								<select id="readingPlace" name="reading_place">
									<option value="HOME"><fmt:message
											key="label.order.home" /></option>
									<option value="HALL"><fmt:message
											key="label.order.hall" /></option>
								</select>
							</div>
						</div>
						<div class="row">
							<input style="width: 200px" class="edit"
								onclick="return orderFormValidation()" type="submit"
								name="order" value="<fmt:message key="button.confirm"/> " />
						</div>
					</form>
				</div>
			</div>
					...
					<div class="row">
						...
						<div class="inputCol">
							<textarea id="subject" name="subject"
								placeholder=" <fmt:message key="label.write"/> "
